...
y[NE,3,xx] = 3.75*2;
s.t. 1collision1v0 {i in BAI,j in K, xx1 in {1..11}, xx2 in {(xx1+1)..12}}:
((x0[i,j,xx1]+RL*cos(theta[i,j,xx1])) - (x0[i,j,xx2]-RL*cos(theta[i,j,xx2])))^2 + ((y0[i,j,xx1]+RL*sin(theta[i,j,xx1])) - (y0[i,j,xx2]-RL*sin(theta[i,j,xx2])))^2 >= 4*(R1^2);
s.t. 1collision0v1 {i in BAI,j in K, xx1 in {1..11}, xx2 in {(xx1+1)..12}}:
((x0[i,j,xx1]-RL*cos(theta[i,j,xx1])) - (x0[i,j,xx2]+RL*cos(theta[i,j,xx2])))^2 + ((y0[i,j,xx1]-RL*sin(theta[i,j,xx1])) - (y0[i,j,xx2]+RL*sin(theta[i,j,xx2])))^2 >= 4*(R1^2);
s.t. 1collision1v1 {i in BAI,j in K, xx1 in {1..11}, xx2 in {(xx1+1)..12}}:
((x0[i,j,xx1]-RL*cos(theta[i,j,xx1])) - (x0[i,j,xx2]-RL*cos(theta[i,j,xx2])))^2 + ((y0[i,j,xx1]-RL*sin(theta[i,j,xx1])) - (y0[i,j,xx2]-RL*sin(theta[i,j,xx2])))^2 >= 4*(R1^2);
s.t. 1collision0v0 {i in BAI,j in K, xx1 in {1..11}, xx2 in {(xx1+1)..12}}:
((x0[i,j,xx1]+RL*cos(theta[i,j,xx1])) - (x0[i,j,xx2]+RL*cos(theta[i,j,xx2])))^2 + ((y0[i,j,xx1]+RL*sin(theta[i,j,xx1])) - (y0[i,j,xx2]+RL*sin(theta[i,j,xx2])))^2 >= 4*(R1^2);
data;
param: PXPY := include PXPY;
param: libai := include libai;
set A1 := include A1;
set A2 := include A2;
set A3 := include A3;
set A4 := include A4;
